...
import { computeRouteMargin } from "@/lib/report-builder-data";
...
export function TableRouteMargin({ shipments, config }: { shipments: Shipment[]; config: SectionConfig }) {
  const allData = computeRouteMargin(shipments);
  ...
  return (
    <Card>
      <CardHeader className="pb-3">
        <CardTitle className="text-sm">Route Margin</CardTitle>
        <p className="text-xs text-text-muted">
          Showing {data.length} of {allData.length} routes
        </p>
      </CardHeader>
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b border-border-default">
                <th className="px-3 py-2 text-left font-medium text-text-muted">Route</th>
            ...
            <tbody>
              {data.map((row) => (
                <tr key={row.route} className="border-b border-border-default last:border-0">
                  <td className="px-3 py-2 text-text-primary font-medium">{row.route}</td>
                  ...
                  <td className={`px-3 py-2 text-right font-medium tabular-nums ${row.marginPct >= 0 ? "text-success-600" : "text-error-600"}`}>
                    {row.marginPct}%
                  </td>
                </tr>
              ))}
